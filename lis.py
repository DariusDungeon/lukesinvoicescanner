...
def convert_to_table(invoice, processing_folder, debug):
    """
    This function takes the results from YOLOv5's detect.py function and build a data frame.
    Additionally the images will be interpreted by OCR and put into the column 'ocr_text'

    Parameters:
    invoice (string): The name of the image without the extension (e.g. INV00000515)
    processing_folder (string): The folder path to the data generated by detect.py including '/exp' subfolder
    debug (boolean): Set True to print the OCR text to the console

    Returns:
    df: A data frame with the columns 'class', 'x', 'y', 'width', 'height', 'conf', 'ocr_text'
    """
    # Read data
    classes = {
        0: "Invoice-line",
...
def convert_table_to_dictionary(df):
    """
    This function validates the provided invoice table and creates a dictionary in case it is valid.

    Parameters:
    df (df): Must be a data frame with the columns 'class', 'x', 'y', 'width', 'height', 'conf', 'ocr_text'

    Returns:
    dict: A dictionary structure in case the invoice is valid, otherwise it returns nothing.
    """
    ...
    total_amount = total_list[0].replace(',', '.').replace(' ','').replace('u20ac','')\
        .replace('u00a2','').replace('u00a9','')
    ...
    if len(df_tax.index) > 0:
        ...
        tax_string = tax_list[0].replace(',', '.').replace(' ','').replace('u20ac','')\
            .replace('u00a2','').replace('u00a9','')
    ...
    for index, row in df_line.iterrows():
        ...
        line_amount = line_list[len(line_list) - 1].replace(',', '.').replace(' ','').replace('u20ac','')\
            .replace('u00a2','').replace('u00a9','')
        regex_list = re.findall(r'\d+(?:\.\d{1,2})?', line_amount)
        if not regex_list:
            continue
        line_amount_decimal = round(Decimal(regex_list[-1]), 2)
